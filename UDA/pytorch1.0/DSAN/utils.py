...
#loading KimiaNet
def load_kimiaNet(pt_model_path, input_size, num_classes, adapt=False):
    model = models.densenet121(pretrained=True)
    num = 0
    for param in model.parameters():
      # total no. of layers = 364
      # for un-freezing denselayer16 choose 353
      if(num<=353):
    ...
    #model_final.load_state_dict(torch.load(pt_model_path, map_location=torch.device(device)))
